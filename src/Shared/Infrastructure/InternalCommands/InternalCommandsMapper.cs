...
public class InternalCommandsMapper : IInternalCommandsMapper
{
    ...
    public string? GetName(Type type) => _internalCommandsMap.TryGetBySecond(type, out var name) ? name : null;

    public Type? GetType(string name) => _internalCommandsMap.TryGetByFirst(name, out var type) ? type : null;
}