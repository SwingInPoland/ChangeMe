...
public class DomainNotificationsMapper : IDomainNotificationsMapper
{
    ...
    public string? GetName(Type type) => _domainNotificationsMap.TryGetBySecond(type, out var name) ? name : null;

    public Type? GetType(string name) => _domainNotificationsMap.TryGetByFirst(name, out var type) ? type : null;
}